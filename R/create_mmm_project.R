...
#' @param modeling_start_date - yyyy-mm-dd format string.
#' @param modeling_end_date - yyyy-mm-dd format string.
...
create_mmm_project <- function(path = "~/Desktop/", FiscalYear = NULL,
  ...
  if (!dir.exists("modeling")) {
    ...
    for (nmp in nmps) {
      ...
      # print(location)
      # dir.create(location)
      # create_salesmodel_script(
      #   NMP = nmp, FiscalYear = FiscalYear,
      #   modeling_start_date = modeling_start_date,
      #   modeling_end_date = modeling_end_date
      # )
